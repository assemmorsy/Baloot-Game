getPlayersByTeamId = async (teamId, atTime) => {
    try {
        ...
        select ptpl.player_id as id , p.name , fp.formats -> 'thumbnail' ->> 'url' as image
        ...
        where  frmp.related_type = 'api::player.player' and ptttl.team_id =  ${teamId} ${atTime !== undefined ? ` and pt.transfered_at < '${atTime}' ` : ""} 

        EXCEPT 

        select ptpl.player_id , p.name  , fp.formats -> 'thumbnail' ->> 'url' as image
        from player_transfers pt
        ...
        where  frmp.related_type = 'api::player.player' and ptftl.team_id =  ${teamId} ${atTime !== undefined ? ` and pt.transfered_at < '${atTime}' ` : ""} ;
        `)

        let captain = await getCaptain(teamId, atTime);

        let players = teamPlayers.rows.map(player => {
